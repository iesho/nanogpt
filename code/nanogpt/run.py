...
if args.mode == 'pretrain':
    ...
    corruption_dataset = dataset.WikiDataset(True, 128, open('nanogpt/data/wiki.txt', encoding='utf-8').read())
    # Make the name dataset
    finetune_dataset = dataset.NameDataset(corruption_dataset,
        open(args.finetune_corpus_path, encoding='utf-8').read())
    
    for _, example in zip(range(4), finetune_dataset):
        
        x, y = example
        print('x:', ''.join([finetune_dataset.itos[int(c)] for c in x]))
        print('y:', ''.join([finetune_dataset.itos[int(c)] for c in y]))
