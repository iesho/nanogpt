...
class BytePairEncoder:
    ...
    def _merge_pair(
        ...
        for chars, count in word_count.items():
            ...
            word_count_new[tuple(chars_new)] = word_count_new.get(tuple(chars_new), 0) + count
            print('word count new',chars_new)
    ...
    def train(
        ...
        for i in range(num_merges):
            ...
            ############# YOUR CODE HERE #############
            most_frequent_pair = max(pair_count.items(), key=lambda x: x[1])[0]
            ##########################################
    ...
    def _merge_pair_for_text(
        ...
        while i < len(text):
            ...
            if (text[i], text[i+1]) == pair_to_merge:
                new_text.append(''.join(pair_to_merge))
                i += 2
            else:
                new_text.append(text[i])
                i +=1
